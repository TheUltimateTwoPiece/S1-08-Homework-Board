...
function formatDueLabel(due: string | null) {
  ...
  return { text: "Due " + format(date, "MMM d"), className: "text-zinc-700 dark:text-zinc-300" };
}
    ...
    >
      <div className="hb-bento-head relative z-[1]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-extrabold tracking-tight text-zinc-950 dark:text-zinc-50">{firstName}&apos;s homework</h2>
            <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">{totalDone} of {top.length} done · tap to open</p>
          </div>
        </div>
      ...
      {top.length === 0 ? (
        <div className="flex h-[calc(100%-56px)] flex-col items-center justify-center text-center">
          <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-700">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6 text-zinc-700 dark:text-zinc-200" aria-hidden="true">
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
              <polyline points="14 2 14 8 20 8" />
            </svg>
          </div>
          <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">All caught up!</p>
          <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-300">No homework waiting for you.</p>
        </div>
      ) : (
              ...
              <li key={post.id} className={"hb-posts-widget-row " + rowClass} style={{ animationDelay: (100 + i * 50) + "ms" }}>
                ...
                <Link href={"/posts/" + post.id} className="min-w-0 flex-1 relative z-[3]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={"truncate text-sm font-bold " + (done ? "line-through text-zinc-700 dark:text-zinc-400" : "text-zinc-950 dark:text-zinc-50")}>{post.title}</span>
